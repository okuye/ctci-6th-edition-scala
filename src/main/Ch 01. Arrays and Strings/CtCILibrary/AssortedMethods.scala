...
object AssortedMethods {
  ...
  def randomBoolean(): Boolean = randomIntInRange(0, 1) == 0

  def randomBoolean(percentTrue: Int): Boolean = randomIntInRange(1, 100) <= percentTrue

  def randomBooleanMatrix(M: Int, N: Int, percentTrue: Int): Array[Array[Boolean]] = {
    Array.tabulate(M, N) { (_, _) => randomBoolean(percentTrue) }
  }

  def randomMatrix(M: Int, N: Int, min: Int, max: Int): Array[Array[Int]] = {
    Array.tabulate(M, N) { (_, _) => randomIntInRange(min, max) }
  }

  def randomArray(N: Int, min: Int, max: Int): Array[Int] = {
    Array.fill(N)(randomIntInRange(min, max))
  }

  def randomLinkedList(N: Int, min: Int, max: Int): LinkedListNode = {
    val root = new LinkedListNode(randomIntInRange(min, max))
    var prev = root
    for (_ <- 1 until N) {
      val next = new LinkedListNode(randomIntInRange(min, max))
      prev.setNext(next)
      prev = next
    }
    root
  ...
  def linkedListWithValue(N: Int, value: Int): LinkedListNode = {
    val root = new LinkedListNode(value)
    var prev = root
    for (_ <- 1 until N) {
      val next = new LinkedListNode(value)
      prev.setNext(next)
      prev = next
    }
    root
  ...
  def createLinkedListFromArray(vals: Array[Int]): LinkedListNode = {
    val head = new LinkedListNode(vals.head)
    vals.tail.foldLeft(head) { (current, v) =>
      val newNode = new LinkedListNode(v)
  ...
  def stringArrayToString(array: Array[String]): String = {
    array.mkString(", ")
  }

  def toFullBinaryString(a: Int): String = {
    Integer.toBinaryString(a).reverse.padTo(32, '0').reverse
  }

  def toBaseNString(a: Int, base: Int): String = {
    Integer.toString(a, base)
  }

  def printMatrix(matrix: Array[Array[Int]]): Unit = {
    matrix.foreach(row => {
      row.foreach(cell => print(f"$cell%4d"))
      println()
    })
  }
  ...
  def printMatrix(matrix: Array[Array[Boolean]]): Unit = {
    matrix.foreach(row => {
      row.foreach(cell => print(if (cell) "1" else "0"))
      println()
    })
  }
  ...
  def charArrayToString(array: Array[Char]): String = {
    array.takeWhile(_ != 0).mkString
  }

  def listOfPointsToString(list: Seq[Point]): String = {
    list.map(p => s"(${p.x},${p.y})").mkString
  }

  def randomBST(N: Int, min: Int, max: Int): TreeNode = {
    val root = new TreeNode(randomIntInRange(min, max))
    for (_ <- 1 until N) {
  ...
  def createTreeFromArray(array: Array[Int]): TreeNode = {
    if (array.isEmpty) return null

    val root = new TreeNode(array.head)
  ...
  def getLongTextBlob: String = {
    val book = "As they rounded a bend in the path that ran beside the river, Lara recognized the silhouette of a fig tree atop a nearby hill. The weather was hot and the days were long. The fig tree was in full leaf, but not yet bearing fruit. " +
    ...
    book.replace('.', ' ').replace(',', ' ').replace('-', ' ')
  }

  def getLongTextBlobAsStringList: Array[String] = getLongTextBlob.split(" ")

  def getTrieDictionary: Trie = new Trie(getListOfWords())

  def getWordListAsHashSet: Set[String] = getListOfWords().toSet

  def getLongSampleText(forceLower: Boolean): String = {
    val text =
      """"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi enim magna, rhoncus at risus at, tincidunt pretium sapien. Maecenas vitae sagittis urna. Donec egestas, mauris ut porttitor eleifend, lectus felis sollicitudin sapien, luctus tempor ligula lacus sit amet quam. Quisque gravida est dolor, vel cursus lacus cursus vel. Morbi vulputate ligula nec libero ullamcorper iaculis. Sed finibus tincidunt scelerisque. Cras condimentum lectus ac sem ornare, non pellentesque enim bibendum. Quisque tincidunt eget elit ut iaculis. Aliquam purus sapien, tincidunt non imperdiet eu, venenatis quis turpis. Vestibulum finibus porta urna sed consequat. Phasellus at rutrum enim. In euismod tellus nec ligula placerat dapibus. Quisque mauris urna, malesuada sit amet scelerisque at, euismod vitae nulla. Sed cursus lacus vitae leo venenatis, id ultrices ex fermentum.\r\n" +
        ...
        |				"\r\n" +
        |				"Donec volutpat, leo auctor congue iaculis, tellus urna feugiat lorem, id laoreet velit sem porttitor diam. Quisque interdum volutpat metus ac pellentesque. Etiam nec nunc sed urna vestibulum posuere ut vitae metus. Duis ac tortor scelerisque, fermentum erat a, blandit purus. Donec ac nunc ipsum. Duis."""".stripMargin

    if (forceLower) text.toLowerCase else text
  }

  def getListOfWords(): Array[String] = {
    Array("the", "of", "and", "a", "to", "in", "is", "be",
